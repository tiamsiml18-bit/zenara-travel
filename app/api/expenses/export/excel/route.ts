...
import { generateExpenseReportExcel } from '@/lib/services/reports-export';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const excelBuffer = generateExpenseReportExcel(records);

    return new NextResponse(new Uint8Array(excelBuffer), {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="expense-report-${new Date().toISOString().slice(0, 10)}.xlsx"`,
      },
    });
  } catch (err) {
