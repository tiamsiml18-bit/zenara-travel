...
export function QuotationWizard({
  ...
  const [trip, setTrip] = useState({
    ...
    supplierCost: (initialData?.supplierCost ?? '') as number | '',
  ...
  function handleSubmit() {
    ...
    const input: QuotationDraftInput = {
      ...
      exclusions,
      itinerary,
      supplierCost: trip.supplierCost === '' ? 0 : Number(trip.supplierCost),
          ...
          <div className="space-y-4">
            ...
            {showPricing && (
              <div className="rounded-md border border-coral-500/30 bg-coral-500/5 p-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <LabeledInput
                    label="Supplier cost (PHP)"
                    type="number"
                    value={String(trip.supplierCost)}
                    onChange={(v) => setTrip((t) => ({ ...t, supplierCost: v === '' ? '' : Number(v) }))}
                  />
                  <LabeledInput
                    label="Markup (PHP)"
                    type="number"
